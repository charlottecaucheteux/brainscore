...
def get_brain_score(
    ...
    if subject == "avg":
        ...
        params = zip(
            df_task.audio_task, df_task.bold_task, df_task.subject, df_task.onset
        )
    ...
    for params_ in params:
        ...
        try:
            if singlesub:
                ...
                # Load bold responses
                gii_fname = f"{subject}_task-{task}_*space-{space}_hemi-{hemi}_desc-clean.func.gii"
                subj_data = get_bold(
    ...
    for k, X in enumerate(features):
        ...
        logging.info(f"Running for layer {layers[k]}")
