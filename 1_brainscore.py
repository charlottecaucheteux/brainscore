...
scratches = ["", "_scratch"]
FEATURES = [
    f"{feature_type}_{scale}{scratch}" for feature_type in types for scale in scales for scratch in scratches]
LAYERS = None
...
if __name__ == "__main__":
    ...
    if AVERAGE_BOLD:
        ...
        for task in task_df.audio_task.unique():
            task_df[task] = task_df["all_tasks"].str.contains(task)
        for task in IGNORE_TASK:
            task_df = task_df.query(f"not {task}")

        # Set subjects and tasks
        subjects = task_df.subject.unique()[:N_SUBJECTS]
    ...
    keys = ["subject", "feature_files",
            "output_file", "hemi", "layers", "to_rois", "x_pca"]
